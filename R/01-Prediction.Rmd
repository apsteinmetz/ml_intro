...
```{r}
#lm_spec <- lm(Lot_Area ~ First_Flr_SF,ames)

lm_spec <- linear_reg(mode = "regression", penalty = NULL, mixture = NULL) %>% set_engine(engine = "lm") 
```
...
ggplot(price_pred,aes(truth,.pred)) + geom_point() + geom_smooth(method="lm")
```
...
ames_split  <- ames %>% initial_split(prop = .75)
...
```{r}
rt_spec <- decision_tree(mode = "regression") %>% 

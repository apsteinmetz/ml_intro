...
cv_folds <- vfold_cv(ames_train, v = 10, strata = Sale_Price, breaks = 4)
knn_wf %>% fit_resamples(resamples = cv_folds) %>% collect_metrics()
```
...
rf_results <- rf_wf %>% tune_grid(resamples = so_folds)
```
...
```{r}
so_best <- rf_results %>% 
  select_best(metric = "roc_auc",maximize = TRUE)
...
so_test_results %>% collect_metrics()
```
